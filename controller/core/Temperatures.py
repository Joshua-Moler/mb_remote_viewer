...
class lakeShoreCom:
    ...
    def pullValues(self):
        if self.ls == None:
            return False
        self.currentTemp = [self.ls.get_kelvin_reading(
            ii) for ii in self.sensors.values()]
        self.currentRes = [self.ls.get_resistance_reading(
            ii) for ii in self.sensors.values()]
        return True
    ...
    def setupWarmupHeater(self, resistance: float, max_power: float, max_voltage=None):
        try:
            ...
            self.ls.setup_warmup_heater(
                resistance, max_current, Model372HeaterOutputUnits.CURRENT)
    ...
    def setupSampleHeater(self, resistance: float):
        if resistance <= 2000:
            try:
                self.ls.setup_sample_heater(
                    resistance, Model372HeaterOutputUnits.POWER)
    ...
    def setSampleHeaterOutputCurrent(self, current: float):
        ...
        try:
            self.ls.set_heater_output_range(
                0, SAMPLE_HEATER_OUTPUT_RANGE[rangeLimit])
...
def check_temperatures():
    ...
    return (_lakeshore_com.pullValues(), {jj: _lakeshore_com.currentTemp[ii] for ii, jj in enumerate(_lakeshore_com.sensors)})


def check_resistances():

    return (_lakeshore_com.pullValues(), {jj: _lakeshore_com.currentRes[ii] for ii, jj in enumerate(_lakeshore_com.sensors)})
...
def check_temperature(p=None):
    ...
    if hasattr(p, '__iter__'):
        returnTemperatures = {ii: temperatures[ii]
                              for ii in p if ii in temperatures}
...
def check_resistance(p=None):
    if p == None:

        # Success if the temperatures were successfully read.
        # Returns the current temperatures of all thermometers.
        return check_resistances()

    success, resistances = check_resistances()
    if hasattr(p, '__iter__'):
        returnResistances = {ii: resistances[ii]
                             for ii in p if ii in resistances}
        success = success and (len(returnResistances) == len(p))

        # Success if requested temperatures were successfully updated.
        # Returns the current values of the requested temperatures.
        return (success, returnResistances)

    returnResistance = resistances[p] if p in resistances else -1
    success = success and returnResistance != -1

    # Success if requested temperature was successfully updated.
    # Returns the requested temperature if the request was valid
    # Returns an error code if the request was invalid:
    #   -1 if the request was not valid
    return (success, returnResistance)
