...
function getInitialCards(cardNum) {
    const cardNames = {
        0: "STATUS",
        1: "TEMPERATURES",
        2: "PRESSURES",
    ...
    return Object.fromEntries([...Array(cardNum).keys()].map((ii) => {
...
export default class CardHolder extends Component {
    constructor(props) {
        ...
        this.cardNum = 7
        this.open = false
        this.cards = getInitialCards(this.cardNum)
        this.totalHeight = 100
        this.activeCard = 1

        const json = props.values
        ...
        for (const card in this.cards) {
            if (json[this.cards[card].name]) {
                ...
                this.cards[card].underGradient = [...Array(Object.keys(this.cards[card].underCardValues).length).keys()].map(
                    (ii) => Object.keys(this.cards[card].underCardValues).length ? generateGradient([255, 255, 255, 0.0], [255, 255, 255, 0.0], Object.keys(this.cards[card].underCardValues).length) : ""
                )
    ...
    static getDerivedStateFromProps(nextProps, prevState) {
        ...
        const json = nextProps.values
        let cards = prevState === undefined ? getInitialCards(this.cardNum) : JSON.parse(JSON.stringify(prevState))
        for (const card in cards) {
            if (json[cards[card].name]) {
                cards[card].underCardValues = JSON.parse(JSON.stringify(json[cards[card].name]))
                const dataLength = Object.keys(json[cards[card].name]).length

                if (cards[card].activeLabel === '')
                    cards[card].activeLabel = dataLength ? Object.keys(json[cards[card].name])[dataLength - 1] : ""
