...
class Response:
    ...
    def getConverterTemperature(self):
        return self.pzd3
...
class opsManager():
    ...
    def update(self):
        if self.serialManager.isOpen():
            ...
            response = {
                "running": self.response.isRunning,
                "stator_frequency": self.response.getStatorFrequency(),
                "converter_temperature": self.response.getConverterTemperature(),
                "motor_current": self.response.getMotorCurrent(),
                "bearing_temperature": self.response.getBearingTemperature(),
                "circuit_voltage": self.response.getCircuitVoltage(),
            }
...
def turbo_ping(turbo):
    if turbo in _turbo_com:
        return _turbo_com[turbo].update()
    else:
        return (False, {})


def turbo_query(turbo, queryParams: dict):
    query = Query()
    if "type" and "number" in queryParams:
        query.setParameterAccess(
            des=queryParams["type"], num=queryParams["number"]
        )
    if "index" in queryParams:
        query.setParameterIndex(queryParams["index"])
    if "value" in queryParams:
        query.setParameterValue(queryParams["value"])

    if _turbo_com[turbo].sendQuery(query):
        response = _turbo_com[turbo].response
        responseParams = {
            "type": response.getParameterAccess(),
            "number": response.getParameterNumber(),
            "index": response.getParameterIndex(),
            "value": response.getParameterValue()
        }
        return (True, responseParams)
    return (False, {"type": 0, "number": 0, "index": 0, "value": 0})
